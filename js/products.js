...
// Summer months (May–Sep): hide fur/winter products from the catalog (getCatalog() в state.js)
function _isWinter(p) {
  const t = `${p.name} ${p.brand}`.toLowerCase();
  return ['fur', 'хутр', 'зим', 'winter', 'шерп', 'термо', 'утепл', 'fleece', 'дублянк', 'пуховик'].some(kw => t.includes(kw));
}
...
function renderPopularRow(data) {
  ...
  const pool = [...data]
    ...
    .sort((a, b) => {
      ...
      const ae = (typeof _hasExtraPhotos === 'function' && _hasExtraPhotos(a)) ? 1 : 0;
      const be = (typeof _hasExtraPhotos === 'function' && _hasExtraPhotos(b)) ? 1 : 0;
...
function renderHomeAllGrid(data) {
  ...
  // Пріоритет показу: 1) знижка, 2) товари з доп.фото (кілька живих знімків), решта — перемішано
  const hasExtra   = typeof _hasExtraPhotos === 'function' ? _hasExtraPhotos : () => false;
  const discounted = data.filter(p => p.oldPrice && p.oldPrice > p.price);
  const regular    = data.filter(p => !p.oldPrice || p.oldPrice <= p.price);
  const regExtra   = regular.filter(hasExtra);
  const regRest    = regular.filter(p => !hasExtra(p));
  _gridData     = [...shuffleSeeded(discounted, 3), ...shuffleSeeded(regExtra, 5), ...shuffleSeeded(regRest, 4)];
  _gridRendered = 0;
  el.innerHTML  = '';
  _renderGridBatch(el);
}
