...
function getCatalog() {
  let all = S.catalog.all || [];
  // Влітку (травень–вересень) прибираємо явно зимові товари (хутро, термо тощо) з каталогу.
  // S.catalog.all лишається повним — карточки в кошику/улюблених/по прямому лінку не ламаються.
  if (typeof _isSummer === 'function' && typeof _isWinter === 'function' && _isSummer()) {
    all = all.filter(p => !_isWinter(p));
  }
