...
// ── ДОП. ФОТО З TELEGRAM (data/extra_photos.json, формат {product_id: [url,...]}) ──
async function _fetchExtraPhotos() {
  try {
    const res = await fetch('data/extra_photos.json?v=' + Math.floor(Date.now() / 300000), { cache: 'no-store' });
    if (!res.ok) return {};
    const json = await res.json();
...
// Домішує доп.фото в product.images (за product.id). Не чіпає ціну і жодні інші поля.
function _mergeExtraPhotos(products, extraPhotos) {
  if (!extraPhotos || !Object.keys(extraPhotos).length) return products;
  return products.map(p => {
    const extra = extraPhotos[p.id];
    if (!extra || !extra.length) return p;
    const images = [...new Set([...(p.images || []), ...extra])];
    return { ...p, images, image: p.image || images[0] };
  });
}

async function bgRefreshCatalog() {
  // Завантажуємо авто-каталог та доп.фото з Telegram паралельно зі статичним
  const autoPromise  = _fetchAutoCatalog();
  const extraPromise = _fetchExtraPhotos();

  // Спроба 1: статичний products.json
  const staticJson        = await _fetchStaticCatalog();
  const autoProducts      = await autoPromise;
  const extraPhotos       = await extraPromise;
  const staticProductsRaw = staticJson ? staticJson.products.map(normalizeProduct) : [];
  const staticProducts    = _mergeExtraPhotos(staticProductsRaw, extraPhotos);
  const merged = staticProducts;

  if (staticProducts.length >= CFG.MIN_PRODUCTS) {
  ...
  try {
    ...
    const gasProducts = _mergeExtraPhotos(raw.map(normalizeProduct), extraPhotos);
    const normalized  = gasProducts;
    ...
    clearTimeout(timer);
    if (false /* TG-канал не є джерелом товарів */) {
